...
can_ok $s, qw( _cache cache clear_cache );

ok !$s->cache, 'no caching by default';

is $s->stem('fooing'), 'foo', 'default stemming';

is_deeply $s->_cache, {}, 'cache is empty when not enabled';

$s->cache(1);

ok $s->cache, 'caching enabled via method';

is_deeply $s->_cache, {}, 'cache is empty by default';

is $s->stem('fooing'), 'foo', 'stemming with caching enabled';

is_deeply $s->_cache, $cache1, 'stem is cached';

is $s->stem('fooing'), 'foo', 'stemming using cache';

# don't try this at home!
$s->_cache->{$s->source}{en}{fooing} = 'goo';
...
is_deeply $s->_cache, {}, 'cache is empty after clearing';
...
is_deeply $s->_cache, {}, 'cache is cleared when disabling';

is $s->stem('fooing'), 'foo', 'stemming after disabling cache';

is_deeply $s->_cache, {}, 'cache not populated after stemming when disabled';
...
is_deeply $s2->_cache, {}, 'cache is empty by default';

is $s2->stem('fooing'), 'foo', 'stemming with caching enabled';

is_deeply $s2->_cache, $cache1, 'stem is cached';
...
is_deeply $s2->_cache, $cache1, 'cache is per object';
is_deeply $s3->_cache, $cache2, 'cache is per object';
